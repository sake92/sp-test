...
class JsonSchemaRepository(xa: Transactor[IO]) {
  ...
  def create(jsonSchema: JsonSchema): IO[Int] =
    sql"""INSERT INTO json_schema (schema_id, content)
          VALUES (${jsonSchema.schemaId}, ${jsonSchema.content})""".update.run.transact(xa)

  def getById(schemaId: String): IO[Option[JsonSchema]] =
    sql"""SELECT schema_id, content FROM json_schema
          WHERE  schema_id = $schemaId""".query[JsonSchema].option.transact(xa)
